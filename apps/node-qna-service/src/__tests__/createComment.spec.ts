import { createCommentRequest, createQuestionRequest } from '../helpers'
...
describe('Create Comment', () => {
  it('returns a 401 when the user is not logged in', async () => {
    ...
    const response = await createCommentRequest(questionId)
  ...
  it('returns a 400 when invalid uuid is provided', async () => {
    ...
    const response = await createCommentRequest('invalid uuid', tokens)
    expect(response.statusCode).toEqual(400)
    expect(response.body.errors).toHaveLength(1)
    expect(response.body.errors[0].message).toEqual('Please provide a valid questionId')
  })

  it('returns a 400 when comment body is not provided', async () => {
    const tokens = global.login()
    const createQuestionResponse = await createQuestionRequest(tokens)
    expect(createQuestionResponse.statusCode).toEqual(201)
    ...
    const response = await createCommentRequest(questionId, tokens, { body: '' })
    expect(response.statusCode).toEqual(400)
    expect(response.body.errors).toHaveLength(1)
  ...
  it('returns a 404 when the question is not found with provided questionId', async () => {
    ...
    const response = await createCommentRequest('7b7a51cf-0876-40d6-aef9-6763a6c74475', tokens)
  ...
  it('returns a 403 when the user is not found', async () => {
    ...
    const response = await createCommentRequest(questionId, user2Tokens)
    expect(response.statusCode).toEqual(403)
    expect(response.body.errors).toHaveLength(1)
    expect(response.body.errors[0].message).toEqual('Action Forbidden')
  })

  it('successfully creates the comment and produces a message to kafka', async () => {
    const tokens = global.login()
    const createQuestionResponse = await createQuestionRequest(tokens)
    expect(createQuestionResponse.statusCode).toEqual(201)
    ...
    const response = await createCommentRequest(questionId, tokens)
